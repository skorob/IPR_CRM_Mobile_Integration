...
public class TestSendOpportunityFromCRMToCRMMobile extends AbstractCrystalJMSTester {

	private static final Logger log = Logger.getLogger(TestSendOpportunityFromCRMToCRMMobile.class);
...
//			this.await();
	...
	public CrystalMessage generateCrystalMessage(int i) {
		...
		message.setQueryName("PUSH_OPPORTUNITY");
		message.setSystemName("CRM");
		try {
			message.setPayload(Resources.toString(Resources.getResource("xml/Opportunity.xml"), Charsets.UTF_8));
		} catch (IOException e) {
			e.printStackTrace();
		}
